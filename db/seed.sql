-- 既存データをクリア（locationはanimeに依存しているので先に削除）
DELETE FROM location;
DELETE FROM anime;

-- ============================================================
-- アニメデータ（都道府県別）
-- ============================================================

-- ************************************************************
-- 北海道地方
-- ************************************************************

-- ------------------------------------------------------------
-- 北海道
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ゴールデンカムイ', 'golden-kamuy', 2018, 'https://x.com/kamuy_anime', 'https://kamuy-anime.com/', '北海道を舞台に、日露戦争帰りの兵士とアイヌの少女が金塊を巡る冒険を繰り広げる。小樽、旭川、網走など北海道各地の実在の風景が多数登場する。', '/images/golden-kamuy.jpg'),
('僕だけがいない街', 'boku-dake-ga-inai-machi', 2016, 'https://x.com/bokumachi_anime', 'https://bokumachi-anime.com/', '北海道苫小牧市を舞台にしたサスペンスアニメ。苫小牧の冬景色や街並みが物語の重要な背景となっている。', '/images/boku-dake-ga-inai-machi.jpg'),
('君に届け', 'kimi-ni-todoke', 2009, 'https://x.com/kimitodo_pr', 'https://kimitodo.com/', '北海道を舞台にした純愛青春アニメ。陰気な見た目から「貞子」と呼ばれる黒沼爽子が、爽やかなクラスメイト・風早翔太との出会いをきっかけに成長していく物語。', '/images/kimi-ni-todoke.jpg');

-- ************************************************************
-- 東北地方
-- ************************************************************

-- ------------------------------------------------------------
-- 青森県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ふらいんぐうぃっち', 'flying-witch', 2016, 'https://x.com/flying_tv', 'https://www.vap.co.jp/flyingwitch/', '青森県弘前市を舞台に、見習い魔女の少女がのんびり暮らす日常アニメ。弘前城や岩木山など弘前の風景が美しく描かれる。', '/images/flying-witch.jpg');

-- ------------------------------------------------------------
-- 岩手県
-- ------------------------------------------------------------
-- ※岩手県が主な舞台のアニメは現在登録なし（聖地のみ：すずめの戸締まり、呪術廻戦）

-- ------------------------------------------------------------
-- 宮城県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ハイキュー!!', 'haikyu', 2014, 'https://x.com/animehaikyu_com', 'https://haikyu.jp/', '宮城県を舞台にした高校バレーボールアニメ。仙台市体育館や県内各地の風景が多数登場する。', '/images/haikyu.jpg');

-- ------------------------------------------------------------
-- 秋田県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ルックバック', 'lookback', 2024, 'https://x.com/lookback_anime', 'https://lookback-anime.com/', '藤本タツキ原作の劇場アニメ。作者の出身地である秋田県にかほ市が舞台のモデルとなっており、田園風景や町並みが忠実に描かれている。', '/images/lookback.jpg');

-- ------------------------------------------------------------
-- 山形県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('好きでも嫌いなあまのじゃく', 'amanojaku', 2024, NULL, 'https://www.amanojaku-movie.com/', 'スタジオコロリド制作の劇場アニメ。山形県米沢市を舞台に、高校生と鬼の少女が母親を探す旅に出る物語。山寺など山形の名所が美しく描かれる。', '/images/amanojaku.jpg'),
('レベルE', 'level-e', 2011, NULL, NULL, '冨樫義博原作のSFコメディアニメ。山形県山形市を舞台に、野球少年と宇宙人王子の奇妙な同居生活を描く。山形市内の街並みが登場する。', '/images/level-e.jpg');

-- ------------------------------------------------------------
-- 福島県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('未確認で進行形', 'mikakunin', 2014, 'https://x.com/mikakunin_anime', 'https://mikakunin.jp/', '荒井チェリー原作のラブコメアニメ。福島県郡山市がモデルで、原作者も福島県出身。雪景色の美しい東北の街並みが描かれる。', '/images/mikakunin.jpg'),
('ざつ旅 -That''s Journey-', 'zatsu-tabi', 2025, NULL, 'https://zatsutabi.com/', '石坂ケンタ原作の旅アニメ。第1話の旅先が福島県会津若松市で、鶴ヶ城や七日町通りなど歴史ある街並みが丁寧に描かれている。', '/images/zatsu-tabi.jpg');

-- ************************************************************
-- 関東地方
-- ************************************************************

-- ------------------------------------------------------------
-- 茨城県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ガールズ＆パンツァー', 'girls-und-panzer', 2012, 'https://x.com/garupan', 'https://girls-und-panzer.jp/', '茨城県大洗町を舞台にした戦車道アニメ。大洗町は作品と一体となった町おこしで有名。', '/images/girls-und-panzer.png');

-- ------------------------------------------------------------
-- 栃木県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('秒速5センチメートル', 'byousoku-5cm', 2007, NULL, 'https://www.cwfilms.jp/5cm/', '新海誠監督のアニメ映画。第一話は栃木県岩舟駅での再会、第三話は鹿児島県種子島が舞台。各地の風景が美しく描かれる。', '/images/byousoku-5cm.jpg');

-- ------------------------------------------------------------
-- 群馬県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('頭文字D', 'initial-d', 1998, NULL, NULL, '群馬県の峠道を舞台にした走り屋アニメ。秋名山（榛名山）、妙義山、赤城山など群馬の峠道が実在のまま描かれる。', '/images/initial-d.jpg'),
('宇宙よりも遠い場所', 'yorimoi', 2018, 'https://x.com/yorimoi', 'http://yorimoi.com/', '群馬県館林市を舞台に南極を目指す少女たちの物語。館林駅やつつじが岡公園など実在の風景が描かれる。', '/images/yorimoi.jpg'),
('日常', 'nichijou', 2011, NULL, 'https://shinonome-lab.com/', 'あらゐけいいち原作のシュールギャグアニメ。群馬県伊勢崎市をモデルにした時定高校を舞台に、ありえないほど壮大な「日常」が繰り広げられる。京都アニメーション制作。', '/images/nichijou.jpg');

-- ------------------------------------------------------------
-- 埼玉県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('らき☆すた', 'lucky-star', 2007, 'https://x.com/AnimeLuckystar', 'https://www.lucky-ch.com/', '埼玉県久喜市の鷲宮神社が聖地として一躍有名になった作品。アニメ聖地巡礼ブームの火付け役。', '/images/lucky-star.jpg'),
('あの日見た花の名前を僕達はまだ知らない。', 'anohana', 2011, 'https://x.com/anohana_project', 'https://www.anohana.jp/', '埼玉県秩父市を舞台にした感動の青春ドラマ。秩父橋や定林寺など多くの聖地が存在する。', '/images/anohana.jpg'),
('ヤマノススメ', 'yama-no-susume', 2013, 'https://x.com/yamanosusume', 'https://www.yamanosusume.com/1st/', '埼玉県飯能市を舞台にした登山アニメ。天覧山や飯能市街地が実在のまま描かれる。', '/images/yama-no-susume.jpg'),
('月がきれい', 'tsuki-ga-kirei', 2017, 'https://x.com/tsukigakirei_tv', 'https://tsukigakirei.jp/', '埼玉県川越市を舞台にした中学生の恋愛アニメ。川越の蔵造りの町並みや氷川神社が美しく描かれる。', '/images/tsuki-ga-kirei.jpg'),
('のんのんびより', 'non-non-biyori', 2013, 'https://x.com/nonnontv', 'https://nonnontv.com/', '埼玉県小川町など田舎の風景をモデルにしたほのぼの日常アニメ。のどかな田園風景が魅力的に描かれる。', '/images/non-non-biyori.jpg'),
('女子高生の無駄づかい', 'joshikousei-no-mudazukai', 2019, 'https://x.com/jyoshimuda', 'https://jyoshimuda.com/', '個性豊かな女子高生たちの「無駄」な日常を描くギャグアニメ。埼玉県の高校を舞台に、「バカ」「ヲタ」「ロボ」などあだ名で呼び合う少女たちの笑える毎日が繰り広げられる。', '/images/joshikousei-no-mudazukai.jpg'),
('ブルーロック', 'blue-lock', 2022, 'https://x.com/BlueLock_PR', 'https://bluelock-pr.com/', 'サッカー日本代表の世界一のストライカーを育成するため、全国から300人のFWを集めた過酷な選抜プログラム「ブルーロック」を描くサッカーアニメ。エゴイズムをテーマにした斬新な内容が話題に。', '/images/blue-lock.png');

-- ------------------------------------------------------------
-- 千葉県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('やはり俺の青春ラブコメはまちがっている。', 'oregairu', 2013, 'https://x.com/anime_oregairu?lang=ja', 'https://www.tbs.co.jp/anime/oregairu/', '千葉市を舞台にした青春ラブコメ。千葉駅、稲毛海浜公園など千葉市内の実在スポットが多数登場する。', '/images/oregairu.jpg'),
('俺の妹がこんなに可愛いわけがない', 'oreimo', 2010, 'https://x.com/oreimo_eromanga/media', 'https://www.oreimo-anime.com/', '平凡な兄・高坂京介と隠れオタクの妹・桐乃の関係を描くラブコメ。千葉市を舞台に千葉モノレールや千葉公園が登場し、秋葉原のオタク文化も作品の重要な要素。', '/images/oreimo.jpg');

-- ------------------------------------------------------------
-- 東京都
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ラブライブ！', 'lovelive', 2013, 'https://x.com/LoveLive_staff', 'https://www.lovelive-anime.jp/', '東京・秋葉原と神田明神を中心に展開するスクールアイドルアニメ。神田明神は聖地として有名。', '/images/lovelive.jpg'),
('呪術廻戦', 'jujutsu-kaisen', 2020, 'https://x.com/animejujutsu', 'https://jujutsukaisen.jp/', '呪いをテーマにしたダークファンタジーバトルアニメ。渋谷を中心に東京都内各地が舞台として描かれ、特に渋谷事変編では渋谷駅周辺が重要な舞台となる。', '/images/jujutsu-kaisen.jpg'),
('天気の子', 'tenki-no-ko', 2019, 'https://x.com/tenkinoko_movie', 'https://tenkinoko.com/', '新海誠監督のアニメ映画。東京の田端駅周辺や六本木ヒルズ、高円寺の気象神社など都内各所が舞台として描かれる。', '/images/tenki-no-ko.jpg'),
('STEINS;GATE', 'steins-gate', 2011, 'https://x.com/SG_anime', 'http://steinsgate.tv/index.html', '東京・秋葉原を舞台にしたSFアニメ。秋葉原ラジオ会館や柳森神社など周辺の街並みが忠実に再現されている。', '/images/steins-gate.jpg'),
('ぼっち・ざ・ろっく！', 'bocchi-the-rock', 2022, 'https://x.com/BTR_anime', 'https://bocchi.rocks/', '東京・下北沢を中心に描かれるバンドアニメ。下北沢のライブハウスや商店街が実在のまま登場する。', '/images/bocchi-the-rock.jpg'),
('四月は君の嘘', 'shigatsu-wa-kimi-no-uso', 2014, NULL, 'https://www.kimiuso.jp/', '東京都練馬区を中心に描かれる音楽青春アニメ。練馬区の公園や街並みが作品のモデルとなっている。', '/images/shigatsu-wa-kimi-no-uso.jpg'),
('3月のライオン', 'sangatsu-no-lion', 2016, 'https://x.com/3_lion', 'https://3lion-anime.com/', '東京都中央区月島を舞台にした将棋アニメ。月島のもんじゃストリートや隅田川沿いの風景が描かれる。', '/images/sangatsu-no-lion.jpg'),
('言の葉の庭', 'kotonoha-no-niwa', 2013, NULL, 'https://kotonohanoniwa.jp/', '新海誠監督のアニメ映画。東京・新宿御苑が主要な舞台で、雨の日の日本庭園が美しく描かれる。', '/images/kotonoha-no-niwa.jpg'),
('リコリス・リコイル', 'lycoris-recoil', 2022, 'https://x.com/lycoris_recoil', 'https://lycoris-recoil.com/', '東京都墨田区を中心に描かれるガンアクションアニメ。東京スカイツリー周辺や浅草の街並みが登場する。', '/images/lycoris-recoil.jpg'),
('デュラララ!!', 'durarara', 2010, 'https://x.com/drrr_anime', 'https://www.durarara.com/', '東京・池袋を舞台にした群像劇。池袋駅東口やサンシャイン60通りなど池袋の街並みが忠実に描かれる。', '/images/durarara.jpg'),
('とある科学の超電磁砲', 'toaru-kagaku-no-railgun', 2009, 'https://x.com/toaru_project', 'https://toaru-project.com/railgun/', '東京都立川市をモデルにした「学園都市」が舞台のSFアニメ。立川駅周辺やモノレールが登場する。', '/images/toaru-kagaku-no-railgun.jpg'),
('かぐや様は告らせたい～天才たちの恋愛頭脳戦～', 'kaguya-sama', 2019, 'https://x.com/anime_kaguya', 'https://kaguya.love/', '秀知院学園の生徒会を舞台にした頭脳戦ラブコメ。天才同士の生徒会長・白銀御行と副会長・四宮かぐやが、プライドをかけて相手に告白させようとする恋愛バトルを繰り広げる。', '/images/kaguya-sama.jpg'),
('五等分の花嫁', 'gotoubun-no-hanayome', 2019, 'https://x.com/5Hanayome_anime', 'https://www.tbs.co.jp/anime/5hanayome/', '貧乏な高校生・上杉風太郎が、大富豪の五つ子姉妹の家庭教師となるラブコメ。中野区を中心に東京の街並みが多数登場し、修学旅行編では京都も舞台となる。', '/images/gotoubun-no-hanayome.jpg'),
('とらドラ！', 'toradora', 2008, NULL, 'https://www.tv-tokyo.co.jp/anime/toradora/', '目つきの悪さから不良と誤解される高須竜児と、小柄だが凶暴な「手乗りタイガー」逢坂大河のドタバタラブコメ。東京都の西東京エリアがモデルとなっている。', '/images/toradora.jpg'),
('おそ松さん', 'osomatsu-san', 2015, 'https://x.com/osomatsu_PR', 'https://osomatsusan.com/', '赤塚不二夫の名作「おそ松くん」の六つ子たちが大人になった姿を描くギャグアニメ。ニートになった松野家の六つ子が繰り広げるカオスな日常が社会現象を巻き起こした。', '/images/osomatsu-san.jpg'),
('CLANNAD', 'clannad', 2007, NULL, 'https://www.tbs.co.jp/clannad/clannad1/', 'Key原作の恋愛アドベンチャーを京都アニメーションがアニメ化。不良少年・岡崎朋也と病弱な少女・古河渚の出会いから始まる、家族の絆を描いた感動の物語。AFTER STORYでは涙なしには見られない展開が待つ。', '/images/clannad.jpg'),
('時々ボソッとロシア語でデレる隣のアーリャさん', 'roshidere', 2024, 'https://x.com/roshidere', 'https://roshidere.com/', 'ロシア人と日本人のハーフの美少女・アーリャが、隣の席の久世政近にロシア語でこっそりデレる学園ラブコメ。実は政近はロシア語が分かるという秘密が面白い。', '/images/roshidere.jpg'),
('彼女、お借りします', 'kanokari', 2020, 'https://x.com/kanokari_anime', 'https://kanokari-official.com/', 'レンタル彼女サービスをきっかけに始まるラブコメ。東京都新宿区や板橋区周辺が舞台で、主人公・木ノ下和也と水原千鶴の関係が少しずつ変化していく。', '/images/kanokari.jpg'),
('僕の心のヤバイやつ', 'bokuyaba', 2023, 'https://x.com/bokuyaba_anime', 'https://bokuyaba-anime.com/', '中二病の陰キャ男子・市川京太郎と学校一の美少女・山田杏奈の不器用な恋を描く青春ラブコメ。東京都の秋川周辺がモデルとなっている。', '/images/bokuyaba.jpg'),
('ホリミヤ', 'horimiya', 2021, 'https://x.com/horimiya_anime', 'https://horimiya-anime.com/', '学校では派手な美少女・堀京子と地味なクラスメイト・宮村伊澄。放課後の意外な素顔をきっかけに距離が縮まっていく青春ラブコメ。', '/images/horimiya.jpg'),
('冴えない彼女の育てかた', 'saekano', 2015, 'https://x.com/saenai_heroine', 'https://saenai.tv/', 'オタク高校生・安芸倫也が、坂道で出会った少女・加藤恵をヒロインにギャルゲー制作を目指す物語。東京都豊島区や練馬区の風景が多数登場する。', '/images/saekano.jpg'),
('ニセコイ', 'nisekoi', 2014, 'https://x.com/nisekoi_k', 'https://www.nisekoi.jp/', 'ヤクザの息子・一条楽とギャングの娘・桐崎千棘が、両組織の抗争を止めるため恋人のふりをするラブコメ。東京都の下町エリアが舞台のモデルとなっている。', '/images/nisekoi.png'),
('干物妹！うまるちゃん', 'himouto-umaru-chan', 2015, 'https://x.com/umr_official', 'https://umaru-ani.me/', '学校では完璧な美少女だが家ではグータラなオタク妹・うまるの二面性を描くギャグコメディ。東京都八王子市周辺がモデルとされ、駅前の風景などが登場する。', '/images/himouto-umaru-chan.jpg'),
('エロマンガ先生', 'eromanga-sensei', 2017, 'https://x.com/oreimo_eromanga/media', 'https://eromanga-sensei.com/', '「俺の妹」の伏見つかさ原作のラブコメ。ラノベ作家の兄と引きこもりイラストレーターの妹が実は仕事上のパートナーだったという物語。東京都内が舞台。', '/images/eromanga-sensei.jpg');

-- ------------------------------------------------------------
-- 神奈川県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('SLAM DUNK', 'slam-dunk', 1993, NULL, NULL, '湘南を舞台にしたバスケットボール漫画・アニメの金字塔。鎌倉高校前駅の踏切は世界中からファンが訪れる聖地。映画「THE FIRST SLAM DUNK」で再び注目を集めた。', '/images/slam-dunk.jpg'),
('TARI TARI', 'tari-tari', 2012, 'https://x.com/taritarianime', 'https://www.ponican.jp/taritari/', '神奈川県藤沢市・江ノ島を舞台にした合唱部アニメ。江ノ電沿線の風景が美しく描かれる。', '/images/tari-tari.jpg'),
('新世紀エヴァンゲリオン', 'evangelion', 1995, NULL, 'https://www.evangelion.co.jp/', '第3新東京市のモデルとなった神奈川県箱根町が聖地。仙石原や芦ノ湖など箱根の風景が随所に登場する。', '/images/evangelion.jpg'),
('青春ブタ野郎はバニーガール先輩の夢を見ない', 'aobuta', 2018, 'https://x.com/aobuta_anime', 'https://ao-buta.com/', '神奈川県藤沢市・湘南エリアを舞台にした青春ファンタジー。藤沢駅や七里ヶ浜、江ノ島が登場する。', '/images/aobuta.jpg');

-- ************************************************************
-- 中部地方
-- ************************************************************

-- ------------------------------------------------------------
-- 新潟県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('Do It Yourself!!', 'do-it-yourself', 2022, 'https://x.com/diy_anime', 'https://diy-anime.com/', '新潟県三条市を舞台にしたオリジナルアニメ。ものづくりの町として知られる三条市で、女子高生たちがDIYに励む日常を描く。', '/images/do-it-yourself.jpg'),
('アイの歌声を聴かせて', 'ai-no-utagoe', 2021, NULL, 'https://ainouta.jp/', '吉浦康裕監督のオリジナル劇場アニメ。新潟県の風景をモデルにした架空都市を舞台に、AIの少女と高校生たちの青春を描く。', '/images/ai-no-utagoe.jpg');

-- ------------------------------------------------------------
-- 富山県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('true tears', 'true-tears', 2008, NULL, 'http://www.truetears.jp/', '富山県南砺市の城端地区を舞台にした恋愛アニメ。城端むぎや祭りとのコラボでも知られる。', '/images/true-tears.jpg');

-- ------------------------------------------------------------
-- 石川県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('花咲くいろは', 'hanasaku-iroha', 2011, NULL, 'https://www.hanasakuiroha.jp/', '石川県の温泉旅館を舞台に、女子高生が仲居として奮闘する物語。湯涌温泉がモデル。', '/images/hanasaku-iroha.jpg'),
('スキップとローファー', 'skip-to-loafer', 2023, 'https://x.com/takamatsumisaki', 'https://skip-and-loafer.com/', '主人公の故郷として石川県能登地方が描かれる青春アニメ。能登の自然豊かな風景が印象的に登場する。', '/images/skip-to-loafer.jpg');

-- ------------------------------------------------------------
-- 福井県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ちはやふる', 'chihayafuru', 2011, NULL, 'https://www.ntv.co.jp/chihayafuru/', '競技かるたを題材にした青春アニメ。主要キャラ綿谷新の故郷として福井県あわら市が描かれ、聖地として有名。', '/images/chihayafuru.jpg');

-- ------------------------------------------------------------
-- 山梨県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ゆるキャン△', 'yurucamp', 2018, 'https://x.com/yurucamp_anime', 'https://yurucamp.jp/', '女子高生たちがキャンプを楽しむアウトドア系アニメ。山梨県を中心に静岡県など実在のキャンプ場が多数登場する。', '/images/yurucamp.jpg'),
('スーパーカブ', 'super-cub', 2021, 'https://x.com/supercub_anime', 'https://supercub-anime.com/', '山梨県北杜市を舞台にしたバイクアニメ。北杜市の自然豊かな風景や通学路が実在のまま描かれる。', '/images/super-cub.jpg');

-- ------------------------------------------------------------
-- 長野県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('サマーウォーズ', 'summer-wars', 2009, NULL, 'https://s-wars.jp/', '長野県上田市を舞台にした細田守監督のアニメ映画。上田城や別所温泉が登場する。', '/images/summer-wars.jpg'),
('咲-Saki-', 'saki', 2009, 'https://x.com/SakiAnime', 'http://www.saki-anime.com/', '長野県を舞台にした麻雀アニメ。長野市の善光寺周辺などが登場する。', '/images/saki.jpg');

-- ------------------------------------------------------------
-- 岐阜県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('氷菓', 'hyouka', 2012, NULL, 'https://www.kyotoanimation.co.jp/kotenbu/', '岐阜県高山市を舞台にした青春ミステリー。高山の古い町並みや神社が美しく描かれる。', '/images/hyouka.jpg'),
('君の名は。', 'kimi-no-na-wa', 2016, 'https://x.com/kiminona_movie', 'https://www.kiminona.com/', '新海誠監督の大ヒット映画。岐阜県飛騨市の飛騨古川駅や気多若宮神社、東京四ツ谷の須賀神社、長野県の諏訪湖が聖地として有名。', '/images/kimi-no-na-wa.jpg'),
('ひぐらしのなく頃に', 'higurashi', 2006, NULL, 'http://www.oyashirosama.com/web/top/', '岐阜県白川郷をモデルにしたミステリーホラー。白川郷の合掌造り集落が作品の舞台「雛見沢」のモデル。', '/images/higurashi.jpg'),
('聲の形', 'koe-no-katachi', 2016, 'https://x.com/koenokatachi_M', 'https://koenokatachi-movie.com/', '京都アニメーション制作の劇場アニメ。かつていじめの加害者だった少年と聴覚障害を持つ少女の再会と贖罪の物語。岐阜県大垣市が聖地として有名で、美登鯉橋や大垣公園など多数の実在スポットが登場する。', '/images/koe-no-katachi.jpg');

-- ------------------------------------------------------------
-- 静岡県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ラブライブ！サンシャイン!!', 'lovelive-sunshine', 2016, 'https://x.com/LoveLive_staff', 'https://www.lovelive-anime.jp/uranohoshi/', '静岡県沼津市を舞台にしたスクールアイドルアニメ。内浦や淡島など沼津の風景が美しく描かれ町おこしにも貢献。', '/images/lovelive-sunshine.jpg');

-- ------------------------------------------------------------
-- 愛知県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('八十亀ちゃんかんさつにっき', 'yatogame-chan', 2019, NULL, 'https://yatogame.nagoya/', '名古屋の文化やあるあるをテーマにしたご当地アニメ。名古屋城、大須商店街、熱田神宮など名古屋の名所が多数登場する。', '/images/yatogame-chan.jpeg');

-- ************************************************************
-- 近畿地方
-- ************************************************************

-- ------------------------------------------------------------
-- 三重県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('凪のあすから', 'nagi-no-asukara', 2013, 'https://x.com/naginoasukara', 'http://www.nagiasu.jp/', '三重県熊野市をモデルにした海辺の町のファンタジー。熊野の海岸線や漁港の風景が幻想的に描かれる。', '/images/nagi-no-asukara.jpg');

-- ------------------------------------------------------------
-- 滋賀県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('けいおん！', 'k-on', 2009, NULL,'https://www.tbs.co.jp/anime/k-on/k-on_tv/index-j.html', '滋賀県豊郷町の豊郷小学校旧校舎がモデルとなった軽音楽部アニメ。校舎は今も聖地として人気。', '/images/k-on.jpg'),
('中二病でも恋がしたい！', 'chuunibyou', 2012, 'https://x.com/anime_chu_2', 'https://www.anime-chu-2.com/', '元中二病の富樫勇太と現役中二病の小鳥遊六花が繰り広げる学園ラブコメ。滋賀県大津市を舞台に、京阪電車の石山坂本線沿線の風景が美しく描かれる。京都アニメーション制作。', '/images/chuunibyou.jpg');

-- ------------------------------------------------------------
-- 京都府
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('有頂天家族', 'uchouten-kazoku', 2013, NULL, 'https://uchoten-anime.com/', '京都を舞台にしたたぬき一家の物語。下鴨神社や出町桝形商店街など京都の名所が多数登場。', '/images/uchouten-kazoku.jpg'),
('響け！ユーフォニアム', 'hibike-euphonium', 2015, 'https://x.com/anime_eupho', 'https://anime-eupho.com/', '京都府宇治市を舞台にした吹奏楽アニメ。宇治橋や大吉山展望台など宇治の風景が美しく描かれる。', '/images/hibike-euphonium.jpg');

-- ------------------------------------------------------------
-- 大阪府
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ジョゼと虎と魚たち', 'jose-tora', 2020, 'https://x.com/joseetora_movie', 'https://joseetora.jp/', '田辺聖子の名作を劇場アニメ化。大阪を舞台に、車椅子の少女ジョゼと大学生の恋を描く。天保山や通天閣など大阪の街並みが美しく描かれる。', '/images/jose-tora.jpg');

-- ------------------------------------------------------------
-- 兵庫県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('涼宮ハルヒの憂鬱', 'haruhi', 2006, 'https://x.com/haruhi_official', 'https://www.haruhi.tv/', '兵庫県西宮市を舞台にした学園SF。西宮北口駅や甲陽園駅周辺など西宮の街並みが忠実に描かれる。', '/images/haruhi.jpg'),
('かげきしょうじょ!!', 'kageki-shoujo', 2021, 'https://x.com/kageki_anime', 'https://kageki-anime.com/', '兵庫県宝塚市をモデルにした歌劇アニメ。宝塚大劇場周辺の街並みが作品の舞台となっている。', '/images/kageki-shoujo.jpg');

-- ------------------------------------------------------------
-- 奈良県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('境界の彼方', 'kyoukai-no-kanata', 2013, 'https://x.com/anime_kyokai', 'https://tv.anime-kyokai.com/', '奈良県をモデルにした学園ファンタジー。奈良の風景が随所に登場する。', '/images/kyoukai-no-kanata.jpg');

-- ------------------------------------------------------------
-- 和歌山県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('サマータイムレンダ', 'summertime-render', 2022, 'https://x.com/summertime_PR', 'https://summertime-anime.com/', '和歌山県の離島を舞台にしたサスペンスアニメ。友ヶ島や加太の風景がモデルとなっている。', '/images/summertime-render.png');

-- ************************************************************
-- 中国地方
-- ************************************************************

-- ------------------------------------------------------------
-- 鳥取県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('Free!', 'free', 2013, NULL, 'https://iwatobi-sc.com/', '鳥取県岩美町をモデルにした水泳アニメ。岩美海岸や浦富海岸の美しい風景が描かれ、聖地巡礼で盛り上がった。', '/images/free.jpg'),
('名探偵コナン', 'detective-conan', 1996, NULL, 'https://www.ytv.co.jp/conan/', '作者・青山剛昌の出身地である鳥取県北栄町が聖地。青山剛昌ふるさと館やコナン通りが人気スポット。', '/images/detective-conan.jpg');

-- ------------------------------------------------------------
-- 島根県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('神在月のこども', 'kamiari-kodomo', 2021, NULL, 'https://kamiari-kodomo.jp/', '旧暦10月に神々が出雲に集う伝承を題材にした劇場アニメ。母を亡くした少女が神話の地・出雲を目指す成長物語。島根県出雲市の風景が美しく描かれる。', '/images/kamiari-kodomo.jpg'),
('SHAMAN KING', 'shaman-king', 2021, NULL, 'https://shamanking-project.com/', '武井宏之原作のシャーマンバトルアニメ（2021年リメイク版）。主人公・麻倉葉の出身地のモデルが島根県出雲市で、出雲の神話的な世界観が作品と深く結びついている。', '/images/shaman-king.jpg');

-- ------------------------------------------------------------
-- 岡山県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('推しが武道館いってくれたら死ぬ', 'oshibudo', 2020, 'https://x.com/anime_oshibudo', 'https://oshibudo.com/', '平尾アウリ原作のアイドルアニメ。岡山市を舞台に地下アイドルとファンの物語を描く。岡山駅や表町商店街など岡山各地がモデルとして登場する。', '/images/oshibudo.jpg'),
('RE-MAIN', 're-main', 2021, NULL, 'https://re-main.net/', 'MAPPA制作の水球アニメ。岡山県倉敷市を舞台に、記憶を失った天才水球少年が廃部寸前の水球部で再び青春を歩む物語。倉敷美観地区が聖地として人気。', '/images/re-main.jpg');

-- ------------------------------------------------------------
-- 広島県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('たまゆら', 'tamayura', 2011, NULL, 'https://tamayura.info/hitotose/', '広島県竹原市を舞台にした写真好きの少女たちの物語。竹原の町並み保存地区が美しく描かれる。', '/images/tamayura.jpg'),
('かみちゅ！', 'kamichu', 2005, NULL, 'https://www.aniplex.co.jp/kamichu/', '広島県尾道市を舞台にした神様になった中学生の日常アニメ。尾道の坂道や渡船が登場する。', '/images/kamichu.jpg');

-- ------------------------------------------------------------
-- 山口県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('シン・エヴァンゲリオン劇場版', 'shin-evangelion', 2021, 'https://x.com/evangelion_co', 'https://www.evangelion.co.jp/', '庵野秀明監督のエヴァンゲリオン完結編。監督の出身地・山口県宇部市がラストシーンに登場し、宇部新川駅は聖地として大人気。宇部市とのコラボイベントも開催されている。', '/images/shin-evangelion.jpg'),
('マイマイ新子と千年の魔法', 'maimai-shinko', 2009, NULL, 'https://www.mai-mai.jp/', '片渕須直監督のアニメ映画。昭和30年代の山口県防府市を舞台に、お転婆な少女・新子と転校生の友情を描く。防府市の国衙跡周辺が物語の重要な舞台。', '/images/maimai-shinko.jpg');

-- ************************************************************
-- 四国地方
-- ************************************************************

-- ------------------------------------------------------------
-- 徳島県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('蟲師 続章', 'mushishi-zokushou', 2014, NULL, 'https://www.mushishi-anime.com/', '漆原友紀原作の幻想アニメ第2期。山深い日本の原風景が描かれ、徳島県三好市の祖谷のかずら橋が作中の橋のモデルとして聖地になっている。', '/images/mushishi-zokushou.jpg'),
('おへんろ。', 'ohenro', 2014, NULL, 'http://www.ufotable.com/ohenro/', 'ufotable制作の四国八十八箇所巡りアニメ。女子高生3人がお遍路を巡る物語で、ufotableの拠点がある徳島市の風景が丁寧に描かれている。', '/images/ohenro.jpg');

-- ------------------------------------------------------------
-- 香川県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('結城友奈は勇者である', 'yuki-yuna', 2014, 'https://x.com/anime_yukiyuna', 'https://yuyuyu.tv/', '香川県観音寺市を舞台にした魔法少女アニメ。銭形砂絵や琴弾公園など実在の風景が登場する。', '/images/yuki-yuna.jpg'),
('からかい上手の高木さん', 'karakai-takagi-san', 2018, 'https://x.com/takagi3_anime', 'https://takagi3.me/1st/', '香川県小豆島を舞台にした青春ラブコメ。土庄港やエンジェルロードなど島の風景が描かれる。', '/images/karakai-takagi-san.jpg');

-- ------------------------------------------------------------
-- 愛媛県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('すずめの戸締まり', 'suzume-no-tojimari', 2022, NULL, 'https://suzume-tojimari-movie.jp/', '新海誠監督のロードムービー。宮崎県日南市から愛媛、神戸、東京へと旅する物語で各地の風景が実在のまま描かれる。', '/images/suzume-no-tojimari.jpg');

-- ------------------------------------------------------------
-- 高知県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('竜とそばかすの姫', 'belle', 2021, NULL, 'https://ryu-to-sobakasu-no-hime.jp/', '細田守監督のアニメ映画。高知県の仁淀川流域が舞台で、主人公・すずが暮らす自然豊かな高知の集落が美しく描かれる。JR伊野駅や浅尾沈下橋が登場する。', '/images/belle.jpg'),
('海がきこえる', 'umi-ga-kikoeru', 1993, NULL, NULL, 'スタジオジブリ制作のアニメ。高知県高知市が主な舞台で、はりまや橋や帯屋町商店街など高知の街並みがリアルに描かれている。高知を代表するアニメ作品。', '/images/umi-ga-kikoeru.jpg'),
('政宗くんのリベンジ', 'masamune-kun-no-revenge', 2017, 'https://x.com/masamune_tv', 'https://masamune-tv.com/1st/', '8年前に令嬢・安達垣愛姫に「豚足」と罵られ振られたデブで冴えない少年・真壁政宗が、激痩せ・イケメン化して帰国し、愛姫を惚れさせてから振る「デッド・オア・ラブ作戦」を実行する復讐ラブコメディ', '/images/masamune-kun-no-revenge.jpg');

-- ************************************************************
-- 九州・沖縄地方
-- ************************************************************

-- ------------------------------------------------------------
-- 福岡県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('鬼滅の刃', 'kimetsu-no-yaiba', 2019, 'https://x.com/kimetsu_off', 'https://kimetsu.com/anime/', '大正時代を舞台にした鬼退治アニメ。福岡県太宰府市の竈門神社が主人公の名前の由来とされ聖地として大人気。東京・浅草の街並みも登場する。', '/images/kimetsu-no-yaiba.jpg'),
('君の膵臓をたべたい', 'kimisui', 2018, 'https://x.com/kimisui_anime', 'https://kimisui-anime.com/', '住野よる原作の感動アニメ映画。膵臓の病気を抱える山内桜良と、クラスメイトの「僕」が秘密を共有しながら過ごす日々を描く。物語の舞台は福岡県がモデルとされている。', '/images/kimisui.jpg');

-- ------------------------------------------------------------
-- 佐賀県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ゾンビランドサガ', 'zombie-land-saga', 2018, 'https://x.com/zombielandsaga', 'https://zombielandsaga.com/', '佐賀県を舞台にしたゾンビアイドルアニメ。唐津城、嬉野温泉、佐賀城など佐賀の名所が多数登場し、佐賀県の知名度向上に貢献した。', '/images/zombie-land-saga.jpg');

-- ------------------------------------------------------------
-- 長崎県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('ばらかもん', 'barakamon', 2014, 'https://x.com/brkmn_anime', 'https://www.barakamon.jp/', '長崎県五島列島を舞台にした書道家の島暮らしアニメ。五島の美しい自然や集落の風景が温かく描かれる。', '/images/barakamon.jpg'),
('色づく世界の明日から', 'iroduku', 2018, 'https://x.com/iroduku_anime', 'http://www.iroduku.jp/', '長崎県長崎市を舞台にした青春ファンタジー。グラバー園や長崎の坂道、港の風景が美しく描かれる。', '/images/iroduku.jpg');

-- ------------------------------------------------------------
-- 熊本県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('夏目友人帳', 'natsume-yuujinchou', 2008, 'https://x.com/NatsumeYujincho', 'https://www.natsume-anime.jp/', '熊本県人吉市を中心とした風景が美しく描かれる妖怪ファンタジー。田園風景や神社が多数登場。', '/images/natsume-yuujinchou.png'),
('放課後ていぼう日誌', 'houkago-teibou-nisshi', 2020, NULL, NULL, '熊本県芦北町を舞台にした釣りアニメ。芦北の海岸や堤防、のどかな港町の風景が描かれる。', '/images/houkago-teibou-nisshi.jpg');

-- ------------------------------------------------------------
-- 大分県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('進撃の巨人', 'shingeki-no-kyojin', 2013, 'https://x.com/anime_shingeki', 'https://shingeki.tv/', '作者・諫山創の出身地である大分県日田市の山間の風景が壁の中の世界のモデルとされる。日田市には進撃の巨人ミュージアムがある。', '/images/shingeki-no-kyojin.jpg');

-- ------------------------------------------------------------
-- 宮崎県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('推しの子', 'oshi-no-ko', 2023, 'https://x.com/anime_oshinoko', 'https://ichigoproduction.com/', '物語の序盤は宮崎県の田舎町が舞台。主人公が産婦人科医として働く宮崎の自然豊かな風景が描かれる。', '/images/oshi-no-ko.jpg');

-- ------------------------------------------------------------
-- 鹿児島県
-- ------------------------------------------------------------
-- ※鹿児島県が舞台のアニメは栃木県で登録済み（秒速5センチメートル：聖地のみ登録）

-- ------------------------------------------------------------
-- 沖縄県
-- ------------------------------------------------------------
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('はるかなレシーブ', 'harukana-receive', 2018, 'https://x.com/harukana_anime', 'https://www.harukana-receive.jp/', '沖縄県を舞台にしたビーチバレーアニメ。那覇市や北谷町のビーチなど沖縄の美しい海岸が描かれる。', '/images/harukana-receive.png'),
('白い砂のアクアトープ', 'aquatope', 2021, 'https://x.com/aquatope_anime', 'https://aquatope-anime.com/', '沖縄県南城市を舞台にした水族館アニメ。南城市の海岸や沖縄の自然が美しく描かれる。', '/images/aquatope.jpg'),
('沖縄で好きになった子が方言すぎてツラすぎる', 'okitsura', 2025, 'https://x.com/okitsura', 'https://okitsura.com/', '沖縄県うるま市を舞台にした方言ラブコメアニメ。東京から沖縄に引っ越してきた高校生が、方言がきつすぎるヒロインに恋をする物語。うるま市の風景やビーチが美しく描かれる。', '/images/okitsura.jpg');

-- ************************************************************
-- その他（特定の舞台なし・複数地域）
-- ************************************************************
INSERT INTO anime (title, slug, year, xurl, officialurl, description, image) VALUES
('みなみけ', 'minami-ke', 2007, NULL, 'https://www.tv-tokyo.co.jp/anime/minami-ke/', '南家の三姉妹・春香、夏奈、千秋の日常を描くコメディアニメ。「この物語はみなみけ3姉妹の平凡な日常を淡々と描くものです」のナレーションが象徴的な日常系の名作。', '/images/minami-ke.jpg'),
('僕は友達が少ない', 'haganai', 2011, 'https://x.com/haganai_anime', 'https://www.tbs.co.jp/anime/haganai/', '友達がいない少年少女が「隣人部」を結成し友達作りに奮闘する学園ラブコメ。金髪ハーフの三日月夜空と残念系美少女・柏崎星奈を中心にドタバタが繰り広げられる。', '/images/haganai.jpg');


-- ============================================================
-- 聖地データ（都道府県別）
-- ============================================================

-- ************************************************************
-- 北海道地方
-- ************************************************************

-- ------------------------------------------------------------
-- 北海道
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ゴールデンカムイ
('hokkaido', (SELECT id FROM anime WHERE slug = 'golden-kamuy'), '小樽運河'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'golden-kamuy'), '旭川第七師団跡'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'golden-kamuy'), '網走監獄'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'golden-kamuy'), '北海道開拓の村'),
-- 僕だけがいない街
('hokkaido', (SELECT id FROM anime WHERE slug = 'boku-dake-ga-inai-machi'), '苫小牧市立美園小学校'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'boku-dake-ga-inai-machi'), '苫小牧市科学センター'),
-- 君に届け
('hokkaido', (SELECT id FROM anime WHERE slug = 'kimi-ni-todoke'), '羽幌神社'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'kimi-ni-todoke'), '羽幌高校'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'kimi-ni-todoke'), '羽幌橋'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'kimi-ni-todoke'), '羽幌港'),
('hokkaido', (SELECT id FROM anime WHERE slug = 'kimi-ni-todoke'), '朝日大橋');

-- ************************************************************
-- 東北地方
-- ************************************************************

-- ------------------------------------------------------------
-- 青森県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ふらいんぐうぃっち
('aomori', (SELECT id FROM anime WHERE slug = 'flying-witch'), '弘前公園'),
('aomori', (SELECT id FROM anime WHERE slug = 'flying-witch'), '岩木山神社'),
('aomori', (SELECT id FROM anime WHERE slug = 'flying-witch'), 'ミズバショウ沼公園');

-- ------------------------------------------------------------
-- 岩手県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- すずめの戸締まり
('iwate', (SELECT id FROM anime WHERE slug = 'suzume-no-tojimari'), '山田町・織笠駅'),
('iwate', (SELECT id FROM anime WHERE slug = 'suzume-no-tojimari'), '山田湾展望広場'),
-- 呪術廻戦
('iwate', (SELECT id FROM anime WHERE slug = 'jujutsu-kaisen'), '正法寺'),
('iwate', (SELECT id FROM anime WHERE slug = 'jujutsu-kaisen'), '盛岡駅');

-- ------------------------------------------------------------
-- 宮城県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ハイキュー!!
('miyagi', (SELECT id FROM anime WHERE slug = 'haikyu'), '岩手県立軽米町高等学校'),
('miyagi', (SELECT id FROM anime WHERE slug = 'haikyu'), '軽米町町民体育館'),
('miyagi', (SELECT id FROM anime WHERE slug = 'haikyu'), 'カメイアリーナ仙台');

-- ------------------------------------------------------------
-- 秋田県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ルックバック
('akita', (SELECT id FROM anime WHERE slug = 'lookback'), 'にかほ市・文林堂書店'),
('akita', (SELECT id FROM anime WHERE slug = 'lookback'), '仁賀保駅'),
('akita', (SELECT id FROM anime WHERE slug = 'lookback'), '仁賀保大橋');

-- ------------------------------------------------------------
-- 山形県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 好きでも嫌いなあまのじゃく
('yamagata', (SELECT id FROM anime WHERE slug = 'amanojaku'), '小野川温泉 宝寿の湯/米沢市'),
('yamagata', (SELECT id FROM anime WHERE slug = 'amanojaku'), '米沢市米沢駅'),
('yamagata', (SELECT id FROM anime WHERE slug = 'amanojaku'), '笹野観音/米沢市'),
-- レベルE
('yamagata', (SELECT id FROM anime WHERE slug = 'level-e'), '山形県立上山明新館高等学校'),
('yamagata', (SELECT id FROM anime WHERE slug = 'level-e'), '山形駅');

-- ------------------------------------------------------------
-- 福島県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 未確認で進行形
('fukushima', (SELECT id FROM anime WHERE slug = 'mikakunin'), '郡山駅'),
('fukushima', (SELECT id FROM anime WHERE slug = 'mikakunin'), '酒蓋公園'),
('fukushima', (SELECT id FROM anime WHERE slug = 'mikakunin'), '福島県立安積高校'),
-- ざつ旅
('fukushima', (SELECT id FROM anime WHERE slug = 'zatsu-tabi'), '会津若松市'),
('fukushima', (SELECT id FROM anime WHERE slug = 'zatsu-tabi'), '会津若松市飯盛山');

-- ************************************************************
-- 関東地方
-- ************************************************************

-- ------------------------------------------------------------
-- 茨城県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ガールズ＆パンツァー
('ibaraki', (SELECT id FROM anime WHERE slug = 'girls-und-panzer'), '大洗磯前神社'),
('ibaraki', (SELECT id FROM anime WHERE slug = 'girls-und-panzer'), '大洗マリンタワー'),
('ibaraki', (SELECT id FROM anime WHERE slug = 'girls-und-panzer'), '大洗まいわい市場');

-- ------------------------------------------------------------
-- 栃木県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 秒速5センチメートル
('tochigi', (SELECT id FROM anime WHERE slug = 'byousoku-5cm'), '小山駅'),
('tochigi', (SELECT id FROM anime WHERE slug = 'byousoku-5cm'), '岩舟駅');

-- ------------------------------------------------------------
-- 群馬県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 頭文字D
('gunma', (SELECT id FROM anime WHERE slug = 'initial-d'), '榛名山（秋名山）'),
('gunma', (SELECT id FROM anime WHERE slug = 'initial-d'), '妙義山'),
('gunma', (SELECT id FROM anime WHERE slug = 'initial-d'), '赤城山'),
-- 宇宙よりも遠い場所
('gunma', (SELECT id FROM anime WHERE slug = 'yorimoi'), '館林駅'),
('gunma', (SELECT id FROM anime WHERE slug = 'yorimoi'), 'つつじが岡公園'),
('gunma', (SELECT id FROM anime WHERE slug = 'yorimoi'), 'LA MANON'),
-- 日常
('gunma', (SELECT id FROM anime WHERE slug = 'nichijou'), '伊勢崎市伊勢崎駅東側'),
('gunma', (SELECT id FROM anime WHERE slug = 'nichijou'), '伊勢崎大手町郵便局'),
('gunma', (SELECT id FROM anime WHERE slug = 'nichijou'), '伊勢崎市柏川沿い');

-- ------------------------------------------------------------
-- 埼玉県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- らき☆すた
('saitama', (SELECT id FROM anime WHERE slug = 'lucky-star'), '鷲宮神社'),
('saitama', (SELECT id FROM anime WHERE slug = 'lucky-star'), '宮前橋'),
('saitama', (SELECT id FROM anime WHERE slug = 'lucky-star'), '春日部共栄高等学校'),
('saitama', (SELECT id FROM anime WHERE slug = 'lucky-star'), '春日部駅'),
-- あの花
('saitama', (SELECT id FROM anime WHERE slug = 'anohana'), '西武秩父駅'),
('saitama', (SELECT id FROM anime WHERE slug = 'anohana'), '秩父神社'),
('saitama', (SELECT id FROM anime WHERE slug = 'anohana'), '秩父まつり会館裏線路脇'),
('saitama', (SELECT id FROM anime WHERE slug = 'anohana'), '定林寺'),
('saitama', (SELECT id FROM anime WHERE slug = 'anohana'), '旧秩父橋'),
-- ヤマノススメ
('saitama', (SELECT id FROM anime WHERE slug = 'yama-no-susume'), '天覧山'),
('saitama', (SELECT id FROM anime WHERE slug = 'yama-no-susume'), '飯能銀座通り商店街'),
('saitama', (SELECT id FROM anime WHERE slug = 'yama-no-susume'), '飯能駅'),
('saitama', (SELECT id FROM anime WHERE slug = 'yama-no-susume'), '東飯能駅'),
-- 月がきれい
('saitama', (SELECT id FROM anime WHERE slug = 'tsuki-ga-kirei'), '本川越駅'),
('saitama', (SELECT id FROM anime WHERE slug = 'tsuki-ga-kirei'), '入間川街道'),
('saitama', (SELECT id FROM anime WHERE slug = 'tsuki-ga-kirei'), '大正浪漫夢通り'),
('saitama', (SELECT id FROM anime WHERE slug = 'tsuki-ga-kirei'), '川越 熊野神社'),
('saitama', (SELECT id FROM anime WHERE slug = 'tsuki-ga-kirei'), '川越 氷川神社'),
-- のんのんびより
('saitama', (SELECT id FROM anime WHERE slug = 'non-non-biyori'), '小川町立小川小学校 下里分校'),
('saitama', (SELECT id FROM anime WHERE slug = 'non-non-biyori'), '小川町立小川小学校 下里分校カフェ Mozart'),
-- 女子高生の無駄づかい
('saitama', (SELECT id FROM anime WHERE slug = 'joshikousei-no-mudazukai'), '東京都立東村山西高等学校'),
('saitama', (SELECT id FROM anime WHERE slug = 'joshikousei-no-mudazukai'), '八坂駅'),
('saitama', (SELECT id FROM anime WHERE slug = 'joshikousei-no-mudazukai'), '東村山中央公園'),
-- ブルーロック
('saitama', (SELECT id FROM anime WHERE slug = 'blue-lock'), '埼玉スタジアム2002');

-- ------------------------------------------------------------
-- 千葉県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- やはり俺の青春ラブコメはまちがっている。
('chiba', (SELECT id FROM anime WHERE slug = 'oregairu'), '千葉市立稲毛高等学校・附属中学校'),
('chiba', (SELECT id FROM anime WHERE slug = 'oregairu'), '千葉市 高洲コミュニティセンター'),
('chiba', (SELECT id FROM anime WHERE slug = 'oregairu'), 'イオン マリンピア店'),
('chiba', (SELECT id FROM anime WHERE slug = 'oregairu'), '稲毛海浜公園'),
('chiba', (SELECT id FROM anime WHERE slug = 'oregairu'), '稲毛陸橋'),
('chiba', (SELECT id FROM anime WHERE slug = 'oregairu'), '稲毛浅間神社'),
-- 俺の妹がこんなに可愛いわけがない
('chiba', (SELECT id FROM anime WHERE slug = 'oreimo'), '千葉県立千葉商業高等学校'),
('chiba', (SELECT id FROM anime WHERE slug = 'oreimo'), '千葉駅'),
('chiba', (SELECT id FROM anime WHERE slug = 'oreimo'), '千葉公園');

-- ------------------------------------------------------------
-- 東京都
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ラブライブ！
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), '秋葉原駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), 'ラジオ会館'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), 'ゲーマーズ本店'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), 'アキバ系アイドルショップ'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), '明神男坂'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), '神田明神'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), '昌平橋'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), '竹むら'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lovelive'), '秋葉原UDX前'),
-- 呪術廻戦
('tokyo', (SELECT id FROM anime WHERE slug = 'jujutsu-kaisen'), '渋谷駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'jujutsu-kaisen'), '渋谷ヒカリエ'),
('tokyo', (SELECT id FROM anime WHERE slug = 'jujutsu-kaisen'), '吉祥寺プラザ'),
('tokyo', (SELECT id FROM anime WHERE slug = 'jujutsu-kaisen'), 'ボートレース多摩川'),
-- 天気の子
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '田端駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '六本木ヒルズ展望台'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '気象神社（高円寺）'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), 'JR東京総合病院（代々木）'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '代々木会館'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '新宿大ガード西'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '朝日稲荷神社（銀座）'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), 'のぞき坂（豊島区高田）'),
('tokyo', (SELECT id FROM anime WHERE slug = 'tenki-no-ko'), '芝公園（港区）'),
-- STEINS;GATE
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '秋葉原ラジオ会館'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '秋葉原駅・電気街口前広場'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '東京タイムズタワー'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '秋葉原駅南高架下'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '神田ふれあい橋'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '柳森神社'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '芳林公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), 'カフェ・メイリッシュ'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '小檜山ビル'),
('tokyo', (SELECT id FROM anime WHERE slug = 'steins-gate'), '牛丼専門サンボ'),
-- ぼっち・ざ・ろっく！
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), '下北沢駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), 'ビレッジバンガード下北沢店'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '竹芝ふ頭'),('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), '下北沢SHELTER'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), 'ADRIFT•reload'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), 'どんぐり広場公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), 'タイムズ 下北沢第8'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), '片瀬江ノ島駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), '江島神社'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bocchi-the-rock'), '江の島シーキャンドル（タワー）'),
-- 四月は君の嘘
('tokyo', (SELECT id FROM anime WHERE slug = 'shigatsu-wa-kimi-no-uso'), '練馬文化センター'),
('tokyo', (SELECT id FROM anime WHERE slug = 'shigatsu-wa-kimi-no-uso'), '練馬高野台駅近くの桜並木'),
('tokyo', (SELECT id FROM anime WHERE slug = 'shigatsu-wa-kimi-no-uso'), '大泉学園駅近くの桜並木'),
('tokyo', (SELECT id FROM anime WHERE slug = 'shigatsu-wa-kimi-no-uso'), '上石神井駅近くの踏切'),
('tokyo', (SELECT id FROM anime WHERE slug = 'shigatsu-wa-kimi-no-uso'), 'ラ・プリムール'),
('tokyo', (SELECT id FROM anime WHERE slug = 'shigatsu-wa-kimi-no-uso'), 'ブッチャーズテーブル'),
-- 3月のライオン
('tokyo', (SELECT id FROM anime WHERE slug = 'sangatsu-no-lion'), '高橋 東京都中央区新川２丁目９−１３'),
('tokyo', (SELECT id FROM anime WHERE slug = 'sangatsu-no-lion'), '千駄ケ谷駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'sangatsu-no-lion'), '鳩森八幡神社'),
('tokyo', (SELECT id FROM anime WHERE slug = 'sangatsu-no-lion'), '日本将棋連盟（公社） 道場課'),
('tokyo', (SELECT id FROM anime WHERE slug = 'sangatsu-no-lion'), '佃公園 佃堀広場'),
('tokyo', (SELECT id FROM anime WHERE slug = 'sangatsu-no-lion'), '佃小橋'),
-- 言の葉の庭
('tokyo', (SELECT id FROM anime WHERE slug = 'kotonoha-no-niwa'), '新宿御苑'),
-- リコリス・リコイル
('tokyo', (SELECT id FROM anime WHERE slug = 'lycoris-recoil'), '錦糸公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lycoris-recoil'), '押上駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lycoris-recoil'), 'すみだ水族館'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lycoris-recoil'), '隅田公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lycoris-recoil'), '東京スカイツリー'),
('tokyo', (SELECT id FROM anime WHERE slug = 'lycoris-recoil'), 'すみだ珈琲(一説)'),
-- デュラララ!!
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), 'サンシャイン60通り'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), 'サンシャイン通り'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), 'ホテルホワイトシティ前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), '六ツ又歩道橋'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), '池袋駅東口前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), '池袋西口公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), '池袋駅地下コンコース'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), 'サンシャイン中央通り'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), '池袋駅前公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), 'カフェ・ベローチェ 東池袋一丁目店前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'durarara'), '東池袋中央公園'),
-- とある科学の超電磁砲
('tokyo', (SELECT id FROM anime WHERE slug = 'toaru-kagaku-no-railgun'), '立日野橋南交差点付近'),
('tokyo', (SELECT id FROM anime WHERE slug = 'toaru-kagaku-no-railgun'), '立川駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'toaru-kagaku-no-railgun'), '多摩センターパルテノン大通り'),
-- かぐや様は告らせたい
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '三田春日神社'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '津国屋前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '新橋駅西口広場(SL広場)'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '東京らあめんタワー 芝大門本店前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '竹芝ふ頭'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kaguya-sama'), '三田国際ビル前'),
-- 五等分の花嫁
('tokyo', (SELECT id FROM anime WHERE slug = 'gotoubun-no-hanayome'), 'ドルチェ＆ガッバーナ 銀座店'),
('tokyo', (SELECT id FROM anime WHERE slug = 'gotoubun-no-hanayome'), 'ザ・ペニンシュラ東京'),
('tokyo', (SELECT id FROM anime WHERE slug = 'gotoubun-no-hanayome'), 'ラウンドワンスタジアム さいたま・栗橋店'),
-- とらドラ！
('tokyo', (SELECT id FROM anime WHERE slug = 'toradora'), 'JR埼京線 北赤羽駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'toradora'), '新河岸川 河川敷'),
('tokyo', (SELECT id FROM anime WHERE slug = 'toradora'), '北赤羽駅 ガード下歩道'),
('tokyo', (SELECT id FROM anime WHERE slug = 'toradora'), '東武東上線 上板橋駅 北口周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'toradora'), '上板南口銀座'),
-- おそ松さん
('tokyo', (SELECT id FROM anime WHERE slug = 'osomatsu-san'), '市ヶ谷フィッシングセンター'),
('tokyo', (SELECT id FROM anime WHERE slug = 'osomatsu-san'), 'ハローワーク上野'),
('tokyo', (SELECT id FROM anime WHERE slug = 'osomatsu-san'), 'スターバックスコーヒー 表参道B-SIDE店'),
('tokyo', (SELECT id FROM anime WHERE slug = 'osomatsu-san'), '築地市場'),
('tokyo', (SELECT id FROM anime WHERE slug = 'osomatsu-san'), '吉祥寺プラザ'),
('tokyo', (SELECT id FROM anime WHERE slug = 'osomatsu-san'), '井の頭公園'),
-- CLANNAD
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), '羽村駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), 'かめのこ児童公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), '羽村動物公園東の交差点'),
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), '狭山池公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), '瑞穂町ビューパーク'),
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), '沼部駅前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'clannad'), '桜坂 (大田区)'),
-- 時々ボソッとロシア語でデレる隣のアーリャさん
('tokyo', (SELECT id FROM anime WHERE slug = 'roshidere'), 'ラゾーナ川崎'),
('tokyo', (SELECT id FROM anime WHERE slug = 'roshidere'), '南河原公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'roshidere'), '溝の口駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'roshidere'), '絶江坂'),
-- 彼女、お借りします
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '成増駅・地下鉄成増駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '板橋区役所前駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '東京農工大学〔練馬大学〕'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '東武練馬駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '練馬区春日町周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '光が丘駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '練馬駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '桜台駅周辺'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kanokari'), '氷川台駅周辺'),
-- 僕の心のヤバイやつ
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), '東急目黒線目黒駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), '目黒不動尊'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), 'マクドナルド 武蔵小山店'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), '洗足駅構内'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), '洗足駅改札前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), 'ファミリーマート 原町二丁目店前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), '碑文谷警察署 原町交番'),
('tokyo', (SELECT id FROM anime WHERE slug = 'bokuyaba'), '世田谷区立奥沢中学校'),
-- ホリミヤ
('tokyo', (SELECT id FROM anime WHERE slug = 'horimiya'), '杉並保育園前'),
('tokyo', (SELECT id FROM anime WHERE slug = 'horimiya'), '杉並区立梅里 中央公園の北側の道'),
('tokyo', (SELECT id FROM anime WHERE slug = 'horimiya'), '荻窪駅南口 南口仲通り'),
-- 冴えない彼女の育てかた
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '都電雑司ヶ谷駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), 'のぞき坂'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '高戸橋'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '富士見坂'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '曙橋'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '旧古河庭園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '三井アウトレットパーク入間'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '学習院下駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '六ツ又陸橋'),
('tokyo', (SELECT id FROM anime WHERE slug = 'saekano'), '東京アニメーションカレッジ専門学校'),
-- ニセコイ
('tokyo', (SELECT id FROM anime WHERE slug = 'nisekoi'), '中野区鷺宮６丁目歩道橋（武蔵台小前）'),
('tokyo', (SELECT id FROM anime WHERE slug = 'nisekoi'), '代々木公園'),
('tokyo', (SELECT id FROM anime WHERE slug = 'nisekoi'), '江戸川区・古川親水公園'),
-- 干物妹！うまるちゃん
('tokyo', (SELECT id FROM anime WHERE slug = 'himouto-umaru-chan'), 'JR八王子駅 北口側'),
('tokyo', (SELECT id FROM anime WHERE slug = 'himouto-umaru-chan'), 'JR新宿駅'),
('tokyo', (SELECT id FROM anime WHERE slug = 'himouto-umaru-chan'), 'ライオン八王子店(閉店)'),
('tokyo', (SELECT id FROM anime WHERE slug = 'himouto-umaru-chan'), '多賀神社'),
-- 俺の妹がこんなに可愛いわけがない（秋葉原）
('tokyo', (SELECT id FROM anime WHERE slug = 'oreimo'), '秋葉原'),
-- 鬼滅の刃（東京）
('tokyo', (SELECT id FROM anime WHERE slug = 'kimetsu-no-yaiba'), '雲取山'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kimetsu-no-yaiba'), '浅草'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kimetsu-no-yaiba'), '吉原遊郭跡'),
-- 君の名は。（東京）
('tokyo', (SELECT id FROM anime WHERE slug = 'kimi-no-na-wa'), '須賀神社（四ツ谷）'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kimi-no-na-wa'), '東京・新宿駅とバスタ新宿'),
('tokyo', (SELECT id FROM anime WHERE slug = 'kimi-no-na-wa'), '東京・新宿警察署裏の信号');

-- ------------------------------------------------------------
-- 神奈川県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- SLAM DUNK
('kanagawa', (SELECT id FROM anime WHERE slug = 'slam-dunk'), '鎌倉高校前駅踏切'),
('kanagawa', (SELECT id FROM anime WHERE slug = 'slam-dunk'), '湘南海岸'),
('kanagawa', (SELECT id FROM anime WHERE slug = 'slam-dunk'), '鎌倉高校前駅'),
('kanagawa', (SELECT id FROM anime WHERE slug = 'slam-dunk'), '江ノ島電鉄'),
-- TARI TARI
('kanagawa', (SELECT id FROM anime WHERE slug = 'tari-tari'), '江ノ島'),
('kanagawa', (SELECT id FROM anime WHERE slug = 'tari-tari'), '鎌倉高校前'),
-- 新世紀エヴァンゲリオン
('kanagawa', (SELECT id FROM anime WHERE slug = 'evangelion'), '箱根仙石原'),
('kanagawa', (SELECT id FROM anime WHERE slug = 'evangelion'), '芦ノ湖'),
-- 青春ブタ野郎はバニーガール先輩の夢を見ない
('kanagawa', (SELECT id FROM anime WHERE slug = 'aobuta'), '藤沢駅'),
('kanagawa', (SELECT id FROM anime WHERE slug = 'aobuta'), '七里ヶ浜');

-- ************************************************************
-- 中部地方
-- ************************************************************

-- ------------------------------------------------------------
-- 新潟県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- Do It Yourself!!
('niigata', (SELECT id FROM anime WHERE slug = 'do-it-yourself'), '三条市・一ノ木戸商店街'),
('niigata', (SELECT id FROM anime WHERE slug = 'do-it-yourself'), '三条市・三条鍛冶道場'),
-- アイの歌声を聴かせて
('niigata', (SELECT id FROM anime WHERE slug = 'ai-no-utagoe'), '長岡市（街並みのモデル）'),
('niigata', (SELECT id FROM anime WHERE slug = 'ai-no-utagoe'), '新潟市周辺');

-- ------------------------------------------------------------
-- 富山県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- true tears
('toyama', (SELECT id FROM anime WHERE slug = 'true-tears'), '城端駅'),
('toyama', (SELECT id FROM anime WHERE slug = 'true-tears'), '善徳寺');

-- ------------------------------------------------------------
-- 石川県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 花咲くいろは
('ishikawa', (SELECT id FROM anime WHERE slug = 'hanasaku-iroha'), '湯涌温泉'),
('ishikawa', (SELECT id FROM anime WHERE slug = 'hanasaku-iroha'), '金沢駅'),
-- スキップとローファー
('ishikawa', (SELECT id FROM anime WHERE slug = 'skip-to-loafer'), '能登地方');

-- ------------------------------------------------------------
-- 福井県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ちはやふる
('fukui', (SELECT id FROM anime WHERE slug = 'chihayafuru'), 'あわら市（綿谷新の故郷）'),
('fukui', (SELECT id FROM anime WHERE slug = 'chihayafuru'), '芦原温泉駅');

-- ------------------------------------------------------------
-- 山梨県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ゆるキャン△
('yamanashi', (SELECT id FROM anime WHERE slug = 'yurucamp'), '本栖湖キャンプ場'),
('yamanashi', (SELECT id FROM anime WHERE slug = 'yurucamp'), '身延駅'),
-- スーパーカブ
('yamanashi', (SELECT id FROM anime WHERE slug = 'super-cub'), '北杜市');

-- ------------------------------------------------------------
-- 長野県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- サマーウォーズ
('nagano', (SELECT id FROM anime WHERE slug = 'summer-wars'), '上田城'),
('nagano', (SELECT id FROM anime WHERE slug = 'summer-wars'), '別所温泉'),
-- 咲-Saki-
('nagano', (SELECT id FROM anime WHERE slug = 'saki'), '善光寺'),
-- 君の名は。（諏訪湖）
('nagano', (SELECT id FROM anime WHERE slug = 'kimi-no-na-wa'), '諏訪湖');

-- ------------------------------------------------------------
-- 岐阜県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 氷菓
('gifu', (SELECT id FROM anime WHERE slug = 'hyouka'), '高山市古い町並み'),
('gifu', (SELECT id FROM anime WHERE slug = 'hyouka'), '日枝神社'),
('gifu', (SELECT id FROM anime WHERE slug = 'hyouka'), '斐太高等学校'),
-- 君の名は。
('gifu', (SELECT id FROM anime WHERE slug = 'kimi-no-na-wa'), '飛騨古川駅'),
-- ひぐらしのなく頃に
('gifu', (SELECT id FROM anime WHERE slug = 'higurashi'), '白川郷'),
-- 聲の形
('gifu', (SELECT id FROM anime WHERE slug = 'koe-no-katachi'), '大垣市・美登鯉橋'),
('gifu', (SELECT id FROM anime WHERE slug = 'koe-no-katachi'), '大垣市・四季の森公園'),
('gifu', (SELECT id FROM anime WHERE slug = 'koe-no-katachi'), '大垣市・大垣公園'),
('gifu', (SELECT id FROM anime WHERE slug = 'koe-no-katachi'), '養老町・養老天命反転地');

-- ------------------------------------------------------------
-- 静岡県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ゆるキャン△
('shizuoka', (SELECT id FROM anime WHERE slug = 'yurucamp'), '浩庵キャンプ場'),
('shizuoka', (SELECT id FROM anime WHERE slug = 'yurucamp'), '三保の松原'),
-- ラブライブ！サンシャイン!!
('shizuoka', (SELECT id FROM anime WHERE slug = 'lovelive-sunshine'), '沼津市内浦'),
('shizuoka', (SELECT id FROM anime WHERE slug = 'lovelive-sunshine'), '淡島');

-- ------------------------------------------------------------
-- 愛知県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 八十亀ちゃんかんさつにっき
('aichi', (SELECT id FROM anime WHERE slug = 'yatogame-chan'), '名古屋城'),
('aichi', (SELECT id FROM anime WHERE slug = 'yatogame-chan'), '大須商店街'),
('aichi', (SELECT id FROM anime WHERE slug = 'yatogame-chan'), '熱田神宮');

-- ************************************************************
-- 近畿地方
-- ************************************************************

-- ------------------------------------------------------------
-- 三重県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 凪のあすから
('mie', (SELECT id FROM anime WHERE slug = 'nagi-no-asukara'), '熊野市'),
('mie', (SELECT id FROM anime WHERE slug = 'nagi-no-asukara'), '熊野灘沿岸');

-- ------------------------------------------------------------
-- 滋賀県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- けいおん！
('shiga', (SELECT id FROM anime WHERE slug = 'k-on'), '豊郷小学校旧校舎群'),
-- 中二病でも恋がしたい！
('shiga', (SELECT id FROM anime WHERE slug = 'chuunibyou'), '大津市・石山駅周辺'),
('shiga', (SELECT id FROM anime WHERE slug = 'chuunibyou'), '大津市・膳所城跡公園'),
('shiga', (SELECT id FROM anime WHERE slug = 'chuunibyou'), '京阪石山坂本線沿線');

-- ------------------------------------------------------------
-- 京都府
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- けいおん！（京都）
('kyoto', (SELECT id FROM anime WHERE slug = 'k-on'), '修学院駅周辺'),
-- 有頂天家族
('kyoto', (SELECT id FROM anime WHERE slug = 'uchouten-kazoku'), '下鴨神社'),
('kyoto', (SELECT id FROM anime WHERE slug = 'uchouten-kazoku'), '出町桝形商店街'),
-- 響け！ユーフォニアム
('kyoto', (SELECT id FROM anime WHERE slug = 'hibike-euphonium'), '宇治橋'),
('kyoto', (SELECT id FROM anime WHERE slug = 'hibike-euphonium'), '大吉山展望台'),
('kyoto', (SELECT id FROM anime WHERE slug = 'hibike-euphonium'), '京阪宇治駅'),
-- 五等分の花嫁（修学旅行編）
('kyoto', (SELECT id FROM anime WHERE slug = 'gotoubun-no-hanayome'), '清水寺（修学旅行編）'),
('kyoto', (SELECT id FROM anime WHERE slug = 'gotoubun-no-hanayome'), '嵐山（修学旅行編）');

-- ------------------------------------------------------------
-- 大阪府
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ジョゼと虎と魚たち
('osaka', (SELECT id FROM anime WHERE slug = 'jose-tora'), '大阪市・天保山（海遊館周辺）'),
('osaka', (SELECT id FROM anime WHERE slug = 'jose-tora'), '大阪市・通天閣周辺'),
-- 名探偵コナン（服部平次）
('osaka', (SELECT id FROM anime WHERE slug = 'detective-conan'), '通天閣（服部平次の舞台）'),
('osaka', (SELECT id FROM anime WHERE slug = 'detective-conan'), '大阪城');

-- ------------------------------------------------------------
-- 兵庫県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 涼宮ハルヒの憂鬱
('hyogo', (SELECT id FROM anime WHERE slug = 'haruhi'), '西宮北口駅'),
('hyogo', (SELECT id FROM anime WHERE slug = 'haruhi'), '甲陽園駅周辺'),
('hyogo', (SELECT id FROM anime WHERE slug = 'haruhi'), '県立西宮北高校'),
-- かげきしょうじょ!!
('hyogo', (SELECT id FROM anime WHERE slug = 'kageki-shoujo'), '宝塚大劇場周辺'),
-- すずめの戸締まり（神戸）
('hyogo', (SELECT id FROM anime WHERE slug = 'suzume-no-tojimari'), '二宮商店街');

-- ------------------------------------------------------------
-- 奈良県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 境界の彼方
('nara', (SELECT id FROM anime WHERE slug = 'kyoukai-no-kanata'), '奈良公園周辺'),
-- 鬼滅の刃
('nara', (SELECT id FROM anime WHERE slug = 'kimetsu-no-yaiba'), '葛木坐火雷神社');

-- ------------------------------------------------------------
-- 和歌山県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- サマータイムレンダ
('wakayama', (SELECT id FROM anime WHERE slug = 'summertime-render'), '友ヶ島'),
('wakayama', (SELECT id FROM anime WHERE slug = 'summertime-render'), '加太海岸');

-- ************************************************************
-- 中国地方
-- ************************************************************

-- ------------------------------------------------------------
-- 鳥取県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- Free!
('tottori', (SELECT id FROM anime WHERE slug = 'free'), '岩美海岸'),
('tottori', (SELECT id FROM anime WHERE slug = 'free'), '浦富海岸'),
-- 名探偵コナン
('tottori', (SELECT id FROM anime WHERE slug = 'detective-conan'), '青山剛昌ふるさと館（北栄町）'),
('tottori', (SELECT id FROM anime WHERE slug = 'detective-conan'), 'コナン通り');

-- ------------------------------------------------------------
-- 島根県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 神在月のこども
('shimane', (SELECT id FROM anime WHERE slug = 'kamiari-kodomo'), '出雲市・出雲大社'),
('shimane', (SELECT id FROM anime WHERE slug = 'kamiari-kodomo'), '出雲市・稲佐の浜'),
-- SHAMAN KING
('shimane', (SELECT id FROM anime WHERE slug = 'shaman-king'), '出雲市・出雲大社周辺'),
('shimane', (SELECT id FROM anime WHERE slug = 'shaman-king'), '出雲市（旧大社町）');

-- ------------------------------------------------------------
-- 岡山県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 推しが武道館いってくれたら死ぬ
('okayama', (SELECT id FROM anime WHERE slug = 'oshibudo'), '岡山市・表町商店街'),
('okayama', (SELECT id FROM anime WHERE slug = 'oshibudo'), '岡山市・岡山城周辺'),
-- RE-MAIN
('okayama', (SELECT id FROM anime WHERE slug = 're-main'), '倉敷市・倉敷美観地区'),
('okayama', (SELECT id FROM anime WHERE slug = 're-main'), '倉敷市・倉敷駅前');

-- ------------------------------------------------------------
-- 広島県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- たまゆら
('hiroshima', (SELECT id FROM anime WHERE slug = 'tamayura'), '竹原町並み保存地区'),
('hiroshima', (SELECT id FROM anime WHERE slug = 'tamayura'), '竹原駅'),
-- かみちゅ！
('hiroshima', (SELECT id FROM anime WHERE slug = 'kamichu'), '尾道市街地'),
('hiroshima', (SELECT id FROM anime WHERE slug = 'kamichu'), '千光寺');

-- ------------------------------------------------------------
-- 山口県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- シン・エヴァンゲリオン劇場版
('yamaguchi', (SELECT id FROM anime WHERE slug = 'shin-evangelion'), '宇部市・宇部新川駅'),
('yamaguchi', (SELECT id FROM anime WHERE slug = 'shin-evangelion'), '宇部市・琴芝駅周辺'),
-- マイマイ新子と千年の魔法
('yamaguchi', (SELECT id FROM anime WHERE slug = 'maimai-shinko'), '防府市・国衙跡（周防国衙跡）'),
('yamaguchi', (SELECT id FROM anime WHERE slug = 'maimai-shinko'), '防府市・佐波川');

-- ************************************************************
-- 四国地方
-- ************************************************************

-- ------------------------------------------------------------
-- 徳島県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 蟲師 続章
('tokushima', (SELECT id FROM anime WHERE slug = 'mushishi-zokushou'), '三好市・祖谷のかずら橋'),
('tokushima', (SELECT id FROM anime WHERE slug = 'mushishi-zokushou'), '三好市・大歩危峡'),
-- おへんろ。
('tokushima', (SELECT id FROM anime WHERE slug = 'ohenro'), '徳島市・眉山'),
('tokushima', (SELECT id FROM anime WHERE slug = 'ohenro'), '徳島市・新町川沿い');

-- ------------------------------------------------------------
-- 香川県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 結城友奈は勇者である
('kagawa', (SELECT id FROM anime WHERE slug = 'yuki-yuna'), '銭形砂絵'),
('kagawa', (SELECT id FROM anime WHERE slug = 'yuki-yuna'), '琴弾公園'),
-- からかい上手の高木さん
('kagawa', (SELECT id FROM anime WHERE slug = 'karakai-takagi-san'), '小豆島土庄港'),
('kagawa', (SELECT id FROM anime WHERE slug = 'karakai-takagi-san'), 'エンジェルロード');

-- ------------------------------------------------------------
-- 愛媛県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- すずめの戸締まり
('ehime', (SELECT id FROM anime WHERE slug = 'suzume-no-tojimari'), '八幡浜港');

-- ------------------------------------------------------------
-- 高知県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 竜とそばかすの姫
('kochi', (SELECT id FROM anime WHERE slug = 'belle'), 'いの町・JR伊野駅'),
('kochi', (SELECT id FROM anime WHERE slug = 'belle'), '仁淀川町・浅尾沈下橋'),
-- 海がきこえる
('kochi', (SELECT id FROM anime WHERE slug = 'umi-ga-kikoeru'), '高知市・はりまや橋'),
('kochi', (SELECT id FROM anime WHERE slug = 'umi-ga-kikoeru'), '高知市・帯屋町商店街'),
-- 政宗くんのリベンジ
('kochi', (SELECT id FROM anime WHERE slug = 'masamune-kun-no-revenge'), '高知市・高知小津高校');

-- ************************************************************
-- 九州・沖縄地方
-- ************************************************************

-- ------------------------------------------------------------
-- 福岡県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 鬼滅の刃
('fukuoka', (SELECT id FROM anime WHERE slug = 'kimetsu-no-yaiba'), '竈門神社（太宰府市）'),
-- 君の膵臓をたべたい
('fukuoka', (SELECT id FROM anime WHERE slug = 'kimisui'), '福岡市内（舞台モデル）'),
('fukuoka', (SELECT id FROM anime WHERE slug = 'kimisui'), '太宰府天満宮');

-- ------------------------------------------------------------
-- 佐賀県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ゾンビランドサガ
('saga', (SELECT id FROM anime WHERE slug = 'zombie-land-saga'), '唐津城'),
('saga', (SELECT id FROM anime WHERE slug = 'zombie-land-saga'), '嬉野温泉'),
('saga', (SELECT id FROM anime WHERE slug = 'zombie-land-saga'), '佐賀城');

-- ------------------------------------------------------------
-- 長崎県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- ばらかもん
('nagasaki', (SELECT id FROM anime WHERE slug = 'barakamon'), '五島列島'),
('nagasaki', (SELECT id FROM anime WHERE slug = 'barakamon'), '福江港'),
-- 色づく世界の明日から
('nagasaki', (SELECT id FROM anime WHERE slug = 'iroduku'), 'グラバー園'),
('nagasaki', (SELECT id FROM anime WHERE slug = 'iroduku'), '長崎市内');

-- ------------------------------------------------------------
-- 熊本県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 夏目友人帳
('kumamoto', (SELECT id FROM anime WHERE slug = 'natsume-yuujinchou'), '人吉駅'),
('kumamoto', (SELECT id FROM anime WHERE slug = 'natsume-yuujinchou'), '大畑駅'),
('kumamoto', (SELECT id FROM anime WHERE slug = 'natsume-yuujinchou'), '田園風景'),
-- 放課後ていぼう日誌
('kumamoto', (SELECT id FROM anime WHERE slug = 'houkago-teibou-nisshi'), '芦北町'),
('kumamoto', (SELECT id FROM anime WHERE slug = 'houkago-teibou-nisshi'), '芦北海岸');

-- ------------------------------------------------------------
-- 大分県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 進撃の巨人
('oita', (SELECT id FROM anime WHERE slug = 'shingeki-no-kyojin'), '大山ダム（キャラクター像）'),
('oita', (SELECT id FROM anime WHERE slug = 'shingeki-no-kyojin'), '進撃の巨人ミュージアム');

-- ------------------------------------------------------------
-- 宮崎県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 推しの子
('miyazaki', (SELECT id FROM anime WHERE slug = 'oshi-no-ko'), '高千穂'),
('miyazaki', (SELECT id FROM anime WHERE slug = 'oshi-no-ko'), '宮崎市内'),
-- すずめの戸締まり
('miyazaki', (SELECT id FROM anime WHERE slug = 'suzume-no-tojimari'), '日南市油津');

-- ------------------------------------------------------------
-- 鹿児島県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- 秒速5センチメートル
('kagoshima', (SELECT id FROM anime WHERE slug = 'byousoku-5cm'), '種子島宇宙センター'),
('kagoshima', (SELECT id FROM anime WHERE slug = 'byousoku-5cm'), '種子島');

-- ------------------------------------------------------------
-- 沖縄県
-- ------------------------------------------------------------
INSERT INTO location (prefecture, anime_id, name) VALUES
-- はるかなレシーブ
('okinawa', (SELECT id FROM anime WHERE slug = 'harukana-receive'), '那覇市ビーチ'),
('okinawa', (SELECT id FROM anime WHERE slug = 'harukana-receive'), '北谷町アラハビーチ'),
-- 白い砂のアクアトープ
('okinawa', (SELECT id FROM anime WHERE slug = 'aquatope'), '南城市'),
-- 沖縄で好きになった子が方言すぎてツラすぎる
('okinawa', (SELECT id FROM anime WHERE slug = 'okitsura'), 'うるま市・具志川ビーチ'),
('okinawa', (SELECT id FROM anime WHERE slug = 'okitsura'), 'うるま市・具志川公民館');
